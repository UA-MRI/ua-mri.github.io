---
layout: profile
name: Ali Bilgin
title: Professor of Biomedical Engineering, Electrical & Computer Engineering, and Medical Imaging; BIO5 Institute Faculty
permalink: /people/ali-bilgin/
description: Professor working on MRI, image processing, and medical imaging

# Profile card data (used for people page)
category: Faculty
order: 1
image: people/ali_bilgin.jpg
image_circular: true
email: bilgin@arizona.edu
website: https://www2.engr.arizona.edu/~bilgin
scholar: https://scholar.google.com/citations?user=I25Yns8AAAAJ
more_info: >
  <p><strong>Research:</strong> MRI, image processing, signal processing, image compression, and machine learning for medical imaging.</p>

profile:
  align: right
  image: people/ali_bilgin.jpg
  image_circular: true
  more_info: >
    <p><strong>Email:</strong> bilgin@arizona.edu</p>
    <p><strong>Research:</strong> MRI, image processing, signal processing, image compression, and machine learning for medical imaging.</p>

social: false
---

## Research Interests

Ali Bilgin's research sits at the intersection of digital signal processing, image processing, image and video compression, and magnetic resonance imaging. His recent work includes accelerated and quantitative MRI, image reconstruction, medical image analysis, and deep learning methods for biomedical imaging.

## Background

Ali Bilgin is an Associate Professor with appointments in Biomedical Engineering, Electrical and Computer Engineering, and Radiology & Imaging Sciences at the University of Arizona. He also serves as Associate Department Head for Graduate Affairs in Biomedical Engineering and is affiliated with the BIO5 Institute.

He received his B.S. in Electronics and Telecommunications Engineering from Istanbul Technical University, his M.S. in Electrical Engineering from San Diego State University, and his Ph.D. in Electrical Engineering, with a minor in Mathematics, from the University of Arizona.


## Professional Profile

In addition to his research and teaching, Ali Bilgin has contributed broadly to the imaging and signal processing communities through editorial roles and conference leadership, including long-term service with the IEEE Data Compression Conference. More information about his group, publications, and professional activities is available on his [faculty webpage](https://www2.engr.arizona.edu/~bilgin/) and [Google Scholar profile](https://scholar.google.com/citations?user=I25Yns8AAAAJ).
